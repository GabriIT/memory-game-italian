  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFlippedCards([]);
        setMatchedCards([]);
        console.log("Cards after setCards:", shuffledCards);
        console.log("Initial flippedCards:", []);
        console.log("Initial matchedCards:", []);
      } catch (error) {
  ...
  return (
    <div className="memory-game">
      ...
      <div className="cards-container">
        {cards.length > 0 ? (
          cards.map((card) => {
            console.log(`Card ${card.id} - flipped: ${flippedCards.includes(card.id)}, matched: ${matchedCards.includes(card.id)}`);
            return (
              <div
                key={card.id}
